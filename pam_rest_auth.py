...
class RESTAuthPAM:
    ...
    def authenticate(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            self.log(
                f"Failed authentication (Status: {response.status_code})", username
            )
            return False
...
def pam_sm_authenticate(pamh: PamHandleProtocol, flags: int, argv: list[str]) -> int:
    try:
        ...
        if not password:
            syslog.syslog(syslog.LOG_WARNING, f"PAM-REST: No password for {username}")
        ...
        if not authenticator.authenticate(username, password):
            syslog.syslog(syslog.LOG_WARNING, f"PAM-REST: Auth failed for {username}")
            return pamh.PAM_AUTH_ERR
